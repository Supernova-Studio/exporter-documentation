...
export function pageOrGroupActiveInContext(
  pageOrGroup: DocumentationPage | DocumentationGroup,
  context: DocumentationPage | DocumentationGroup
) {
...
export function firstPageFromTop(
  documentationRoot: DocumentationGroup
...
export function firstTabGroupFromTop(
  documentationRoot: DocumentationGroup
): string[] | undefined {
  let pageCount = 0;
  for (let child of documentationRoot.children) {
    if (isExportable(child as DocumentationPage | DocumentationGroup)) {
      if (
        pageCount === 0 &&
        child.type === 'Group' &&
        (child as DocumentationGroup).groupBehavior === 'Tabs'
      ) {
...
export function flattenedPageStructure(
  root: DocumentationGroup
): Array<DocumentationPage> {
  ...
  for (let item of root.children) {
    if (isExportable(item as DocumentationPage | DocumentationGroup)) {
      if (item.type === 'Page') {
        ...
        pages = pages.concat(
          flattenedPageStructure(item as DocumentationGroup)
        );
      }
    }
  }
