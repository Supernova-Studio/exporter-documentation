...
function isSandboxDefinition(block: DocumentationPageBlockCode): boolean {

  let string = block.text.spans.map(s => s.text).join("")
  return string.startsWith("Mode:SANDBOX")
}

function encodeSandboxData(block: DocumentationPageBlockCode): string {
...
function getFrontendSandboxData(block: DocumentationPageBlockCode): {
  code: string,
